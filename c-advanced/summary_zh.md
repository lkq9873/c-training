...
> 这个「read → 补 '\0' → printf」的三件套，在課題12〜16 每一题都会出现。
>
> 讲师 review 時曾指出「文字列の終端のヌルチェック」，一度改成过
> 「发送方连 `'\0'` 一起发 → 接收方**检查**最后一字节」的方式，
> 但**最终又改回了这个三件套**。经过详见[第四部分](#第四部分讲师-review-指摘与对应)。
...
> 检讨经过和最终采用的写法详见[第四部分](#第四部分讲师-review-指摘与对应)。
> ⚠️ 注意：**課題13 目前还是「継続 + ヌルチェック」的中间版本**，
> 課題14〜16 已经改成「打ち切り + 複数 return」了，两边写法不一致。
...
課題3〜16 里出现过两种写法，**不是随意混用，各有适用场景**。

### 模式A：単一 exitStatus + 嵌套 if-else（単一 return）
...
**用在**：課題9〜13、以及**自作関数**（讲师要求「自作関数は入口1個・出口1個」）。

### 模式B：複数 return + 返回前先做后处理（打ち切り）
...
| return 的个数 | 1 个 | 多个 |
| 嵌套深度 | 每多一步深一层 | **永远 0 层** |
| 后处理写在哪 | 集中在外层的 else 里，**写一次** | 每个返回点各写一次，**会重复** |
| 適用 | **自作関数**（規約で出口1個が必須）<br>手順が 3〜4 段まで | **main**（規約で複数 return が許可）<br>手順が 5 段以上 |

**判断标准**：

- **自作関数は必ず模式A** —— 規約が入口1個・出口1個を求めるため、選択の余地がない
- **main は手順の段数で決める** —— 課題15/16 のサーバーのように 6 段ある場合、模式A だと 6 層のネストになるので模式B にする

### 变量命名も合わせて整理した

課題14〜16 では `s4_mkfifoResult` / `s4_readResult` / `s4_closeResult` … を
**`s4_callResult` 1 個に統一**した。同時に使う必要がないため、変数を増やす意味がない。

ただしファイル記述子（`s4_fileDescriptor` / `s4_acceptDescriptor`）は
**複数の呼び出しをまたいで保持する必要がある**ので別変数のまま。

# 第四部分：讲师 review 指摘与对应

課題13 的 review 收到了 2 条 comment。检讨过程比较长，**中间做过一版又改回来了**，这里把经过和最终结论都记下来。

> **最终状态**（2026.08.02 時点）
>
> | | 課題12 | 課題13 | 課題14〜16 |
> |---|---|---|---|
> | エラー時 | 継続 | 継続 | **打ち切り（後始末してから return）** |
> | 文字列の終端 | 旧方式（受信側で `'\0'` 付与） | ヌルチェック | **旧方式（受信側で `'\0'` 付与）** |
> | main の return | 1 個 | 1 個 | **複数個** |
> | 変数名 | 個別 `xxxResult` | 個別 `xxxResult` | **`s4_callResult` に統一** |
>
> ⚠️ **課題12・13 はまだ中間版のまま。** 統一するなら同じ形に直す必要がある。
...
| 方针 | 术语 | 典型领域 | 逻辑 |
|---|---|---|---|
| **打ち切り** | **フェールセーフ**（fail-safe） | 車載、鉄道、医療機器、プラント制御 | 用错误的数据继续跑可能造成**物理伤害**。「動かないほうがマシ」 |
...
理由は技术性的，做过实验验证。把子进程改成「write 之后立刻打ち切り（不 read 直接结束）」：
...
原因在教材表20：**`SIGPIPE`（13）= 読み手のいないパイプへの書き込み**。子进程提前退出 → 读端全关 → 父进程 `write` → SIGPIPE → 默认终止进程。而且输出重定向到管道时（全缓冲），父进程连**已经 printf 出来的内容都会丢**。

**结论：在双向通信里，单方面提前退出会把错误传染给对方。** 这个结论现在依然成立，是个值得记住的知识点。

### 最終的な対応：「打ち切り」に変更した

課題14〜16 では**打ち切り**を採用。ただし単純な打ち切りではなく：

> **エラーを検出したら、それまでに獲得した資源の後始末を行ってから return する。**
...
**为什么 SIGPIPE 的问题不再出现**：課題14 是单向（送信側と受信側が別プログラム）、課題15/16 は accept 後の 1 往復だけなので、「相手を待たせたまま先に落ちる」状況にならない。課題13（双方向パイプ）だけは事情が違うので、**13 は継続のまま残している**。

### ❌ 使ってはいけない理由

> ~~「構造化プログラミングで関数内の return を1個にしているため、途中で打ち切れなかった」~~

**这个说法站不住脚。** 実測：task13 を「単一 exit / 単一 return のまま打ち切り」に書き換えても、
...
只有 2 层嵌套，一点都不深。而且 `structured_programming.md` 里演示「単一 return」的示例代码**本身就是打ち切りの書き方**。

「単一 return」真正影响的是**代码的形状**，不是**方针的选择**。

### 给讲师的说法

> エラー時は**打ち切り**とし、それまでに獲得した資源（ファイル記述子、名前つきパイプ、ソケット）の後始末を行ってから EXIT_FAILURE を返す方針としました。
>
> main 関数は手順が 5〜6 段あり、単一 return のままだと同じ段数のネストになってしまうため、規約の「複数個の return を使用した方が可読性が高まる場合に限り許可する」に基づき複数個の return を使用しています。フロー図にも複数個の return を明記しました。
>
> 一方、自作関数は入口1個・出口1個とするため、戻り値を変数に保持して関数末尾で1個の return により返しています。
...
原来的代码做的是「**付ける**」（强制补终端符），不是「**チェックする**」（检查）：

```c
read(fd, buf, BUFFER_SIZE - 1);
buf[読んだバイト数] = '\0';       /* ← 不管收到什么，最后都塞一个 '\0' */
printf("%s", buf);
```

### 検討の経緯：一度は「送って検査する」方式にした

**寄信的比喻**：

- 旧方式 = 寄信人**不写「以上」**，收信人**自己在收到的纸最后画个句号** → 不管收到整封信还是半张纸，看起来都「完整」
- 新方式 = 寄信人在信末尾**写上「以上」**，收信人**检查最后有没有「以上」** → 没有就知道被撕掉了

实测（消息加长到 147 字节，缓冲区只能收 99）：

| 场景 | 旧方式 | 「送って検査」方式 |
|---|---|---|
| 正常系 | 正常 / 退出码 0 | 正常 / 退出码 0 |
| 超过缓冲区 | **静默截断 / 退出码 0** | `受信文字列が終端されていません` / 退出码 1 |
| 对端不发数据就关闭 | 打印空字符串 | `〜がクローズしました` / 退出码 1 |

**这个知识点本身是对的**：管道和流套接字都是**字节流**，**没有「消息边界」**。`read` 只告诉你读到几个字节，不告诉你这是不是一条完整的消息。划分边界的标准手法有三种：

| 手法 | 说明 |
|---|---|
| **終端文字（デリミタ）方式** | `'\0'` 或 `'\n'` 一起发，接收方检查 |
| **長さ先行（length-prefix）方式** | 先发 4 字节长度再发正文。实务上最正规 |
| **固定長方式** | 消息长度固定 |

### 最終的な対応：旧方式に戻した

課題14〜16 は**旧方式**（送信側は `sizeof(MSG) - 1`、受信側で `buf[n] = '\0'`）に戻した。

```c
#define SEND_MESSAGE_BYTE   (sizeof(SEND_MESSAGE) - 1)   /* 終端のヌル文字を含まない */
...
s4_callResult = read(fd, ch_receiveBuffer, BUFFER_SIZE - 1);   /* '\0' の分を空ける */
...
ch_receiveBuffer[s4_callResult] = '\0';
```

各ファイル冒頭の「設計方針」ブロックに、**この取り決めを明記**している：
...
つまり「無意識にそう書いた」のではなく「**送信側と受信側の取り決めとして明記した**」形にすることで、指摘に対応している。

> 課題13 だけは「送って検査する」方式のまま残っている。

---

## 3. 修改范围一览

| 文件 | エラー時 | 文字列の終端 | main の return | 変数名 |
|---|---|---|---|---|
| task12.c | 継続 | 旧方式 | 1 個 | 個別 |
| task13.c | 継続 | ヌルチェック | 1 個 | 個別 |
| **task14_receive.c / task14_send.c** | **打ち切り** | **旧方式** | **複数** | **`s4_callResult`** |
| **task15_client.c / task16_client.c** | **打ち切り** | **旧方式** | **複数** | **`s4_callResult`** |
| **task15_server.c / task16_server.c** | **打ち切り** | **旧方式** | **複数**（関数は 1 個） | **`s4_callResult`** |

流程图侧も同じ構成に描き直した：

- **main ページ** — 一本の主流れ ＋ 右側にエラー分岐、各分岐の末尾が `EXIT_FAILUREを返す` で、全部が同じ `終了` に合流する
- **関数ページ** — ネスト構造、末尾は `s4_returnValueを返す` の 1 個だけ
- 「戻り値」欄に `※ nestを浅くするため複数個のreturnを使用している` /
  `※ 入口1個・出口1個とするためreturnは1個のみ` と明記

規約「フロー図中で複数個の return を使用したことが明確に分かるようにする」への対応。
...
| `read` / `write` | **实际字节数**（read 返回 **0 = EOF**） | -1 |
...
| 12 | 单向管道 | `pipe` ×1 | 1 |
| 13 | 双向管道 | `pipe` ×2（注意死锁） | 1 |
| 14 | 命名管道 | `mkfifo` + `unlink` | **2** |
